...
async (conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        let privacyMenu = `╭━━〔 *AWAIS-MD PRIVACY SETTING* 〕━━┈⊷
┃◈╭─────────────·๏
...
┃◈└───────────┈⊷
╰──────────────┈⊷
 > *ᴘᴏᴡᴅᴇʀᴇᴅ ʙʏ ᴀᴡᴀɪs ᴍᴅ*`;

        await conn.sendMessage(
            from,
            {
                image: { url: `https://github.com/Awais-star-a11y/TESTING-REPO/raw/refs/heads/main/IMG-20250409-WA0093.jpg` }, // Replace with privacy-themed image if available
                caption: privacyMenu,
                contextInfo: {
...
async (conn, mek, m, { from, isOwner, reply }) => {
    ...
    try {
        ...
        reply(`*📋 AWAIS-MD BLOCKED USERS 👥 FIND ! (${count}):*\n\n${list}`);
    } catch (err) {
        console.error(err);
...
cmd({
    ...
    try {
        ...
        return reply(`AWAIS-MD BIO FIND! USER  BIO:\n\n${about.status}`);
    } catch (error) {
